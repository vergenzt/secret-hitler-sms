...
legislate() {
  ...
  PUBLIC_PRESIDENT_PHONE=`lookup "$PUBLIC_PLAYER_INFO" "$PUBLIC_PRESIDENT_NAME" 3`

  PUBLIC_PRESIDENT_PHONE=`echo "$PUBLIC_PLAYER_INFO" | grep "$PUBLIC_PRESIDENT_NAME" | awk '{print $2}'`
  PUBLIC_PRESIDENT_PREFIX=`echo "$PUBLIC_PLAYER_INFO" | grep "$PUBLIC_PRESIDENT_NAME" | awk '{print $3}'`
  PUBLIC_CHANCELLOR_PHONE=`echo "$PUBLIC_PLAYER_INFO" | grep "$PUBLIC_CHANCELLOR_NAME" | awk '{print $2}'`
  PUBLIC_CHANCELLOR_PREFIX=`echo "$PUBLIC_PLAYER_INFO" | grep "$PUBLIC_CHANCELLOR_NAME" | awk '{print $3}'`
